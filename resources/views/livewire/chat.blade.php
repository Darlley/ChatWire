                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg flex" style="min-height: 400px; max-height: 400px;">
                    ...
                    <div class="w-9/12 md:w-9/12 flex flex-col justify-between">
                        ...
                        <div class="w-full p-6 flex flex-col overflow-y-auto list-scroll text-white">
                            ...
                            <div class="w-full mb-3 @if($list_message['from_user_id'] == Auth::id()) text-right @endif">
                                <p class="@if($list_message['from_user_id'] == Auth::id()) bg-teal-600 @else bg-gray-300 @endif inline-block p-2 rounded-md messageFromMe" style="max-width: 75%;">{{$list_message['content']}}</p>
                                <span class="block mt-1 text-xs text-gray-500">{{$list_message['created_at']}}</span>
                            </div>
                            @endforeach
                            ...
                            <div class="w-full mb-3 text-right">
                                ...
                                <span class="block mt-1 text-xs text-gray-500">{{ $data_message['created_at'] }}</span>
                            </div>
                            @endforeach
                        ...
                        <!-- form -->
                        <div class="w-full flex justify-between items-center bg-gray-300 p-2 border-t border-gray-200" style="bottom: 0px;">
                            <textarea disabled
                            class="flex-grow m-2 py-2 px-4 mr-1 rounded-full border border-gray-200 bg-gray-200 resize-none"
                            rows="1"
                            placeholder="Message..."
                            style="outline: none;"
                            ></textarea>
                            <button
                                class="mr-2 p-2 cursor-default" style="outline: none;"
                            >
                                <svg
                                    class="svg-inline--fa text-white bg-gray-200 fa-paper-plane fa-w-16 w-12 h-12 p-3 rounded-full"
                                    aria-hidden="true"
                                    focusable="false"
                                    data-prefix="fas"
                                    data-icon="paper-plane"
